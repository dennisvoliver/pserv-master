...
socket_open_bind_listen(char * port_number_string, int backlog)
{
    ...
    if (rc != 0) {
        if (!silent_mode) fprintf(stderr, "getaddrinfo error: %s\n", gai_strerror(rc));
        return -1;
    }
    ...
    for (pinfo = info; pinfo; pinfo = pinfo->ai_next) {
        ...
        int rc = getnameinfo(pinfo->ai_addr, pinfo->ai_addrlen,
                             ...
                             NI_NUMERICHOST);
        if (rc != 0) {
            if (!silent_mode) fprintf(stderr, "getnameinfo error: %s\n", gai_strerror(rc));
            return -1;
        }
         ...
         */
/*
        if (pinfo->ai_family != AF_INET) continue;
*/
	// skip IPV4
        if (pinfo->ai_family == AF_INET) continue;
	/*
        if (!silent_mode) printf("%s: %s\n", pinfo->ai_family == AF_INET ? "AF_INET" :
    ...
    if (!silent_mode) fprintf(stderr, "No suitable address to bind port %s found\n", port_number_string);
    return -1;
}
...
socket_accept_client(int accepting_socket)
{
    ...
    if (!silent_mode) {
        ...
        if (!silent_mode) fprintf(stderr, "Accepted connection from %s:%s\n", peer_addr, peer_port);
    }
    return client;
}
